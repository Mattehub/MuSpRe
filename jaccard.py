...
#selecting only the channels we want, in this script H
    ...
    for n in np.arange(int(len(zdata_rest)/10), int(len(zdata_rest)/4)):
        ...
        for i in indices15:
            ...
            jaccard_list_music.append(np.mean(jaccard_l))
        
        
        jaccard_list[subject].append([np.mean(jaccard_list_rest), np.mean(jaccard_list_music), np.mean(jaccard_list_speech)])
...
for subject in subject_list:
    plt.plot(np.mean(np.array(jaccard_list[subject]), axis=0))
